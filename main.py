...
st.subheader("Flight Instruments")
...
with col2:
    thrust_margin = Thrust_available - Thrust_required
    fig2 = go.Figure(go.Indicator(
        mode="gauge+number",
        value=thrust_margin/1000,
...
st.subheader("Runway Visualization")

runway_length = 4000  # 4 km reference runway

takeoff_percent = min(S_takeoff/runway_length,1)
landing_percent = min(S_landing/runway_length,1)

st.progress(takeoff_percent)
st.write(f"Takeoff Distance Used: {round(S_takeoff)} m")

st.progress(landing_percent)
